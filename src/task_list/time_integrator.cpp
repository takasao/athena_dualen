...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    if (DUAL_ENERGY_ENABLED) {
      if (NSCALARS > 0) {
        if (MAGNETIC_FIELDS_ENABLED) {
          AddTask(DUALEN,(INT_HYD|INT_FLD));
        } else {
          AddTask(DUALEN,(INT_HYD));
        }
        AddTask(SRC_TERM,(DUALEN|INT_SCLR));
      } else {
        if (MAGNETIC_FIELDS_ENABLED) {
          AddTask(DUALEN,(INT_HYD|INT_FLD));
        } else {
...
void TimeIntegratorTaskList::AddTask(const TaskID& id, const TaskID& dep) {
  ...
  if (id == CLEAR_ALLBND) {
    ...
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::DualEnergy);
    task_list_[ntasks].lb_time = true;
  } else if (id == SRC_TERM) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
...
//----------------------------------------------------------------------------------------
//! Functions for dual energy formalism

TaskStatus TimeIntegratorTaskList::DualEnergy(MeshBlock *pmb, int stage) {
  Hydro *ph = pmb->phydro;

  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ph->ApplyDualEnergyFormalism(ph->u);
    }
    return TaskStatus::next;
  }
  return TaskStatus::fail;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      ph->hsrc.AddSourceTerms(t_start_stage, dt, ph->flux, ph->w, ps->r, pf->bcc,
                                   ph->u, ps->s);
      // Synchronize ein with etot
      if (DUAL_ENERGY_ENABLED && DUAL_ENERGY_FLAG == 1) {
          // bcc must be updated before calling SynchronizeEintWithEtot
          if (MAGNETIC_FIELDS_ENABLED)
            pf->CalculateCellCenteredField(pf->b,pf->bcc,pmb->pcoord,
                    pmb->is,pmb->ie,pmb->js,pmb->je,pmb->ks,pmb->ke);

          ph->SynchronizeEintWithEtot(ph->w, ph->u, pf->bcc,
                  pmb->is, pmb->ie, pmb->js, pmb->je, pmb->ks, pmb->ke);
      }
    }
    return TaskStatus::next;
