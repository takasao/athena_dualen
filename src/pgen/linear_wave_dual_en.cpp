...
#error "This problem generator requires dual energy formalism"
#endif

namespace {
// CAUTION: necessary preparation for dual energy formalism
static const int NHYDRO0 = NHYDRO-1;
static const int NWAVE0 = NWAVE-1;
...
Real ev[NWAVE0], rem[NWAVE0][NWAVE0], lem[NWAVE0][NWAVE0];
...
void Eigensystem(const Real d, const Real v1, const Real v2, const Real v3,
                 ...
                 const Real x, const Real y, Real eigenvalues[(NWAVE0)],
                 Real right_eigenmatrix[(NWAVE0)][(NWAVE0)],
                 Real left_eigenmatrix[(NWAVE0)][(NWAVE0)]);
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  Real l1_err[NHYDRO0+NFIELD]{}, max_err[NHYDRO0+NFIELD]{};
  ...
  for (int b=0; b<nblocal; ++b) {
        ...
        for (int i=il; i<=iu; i++) {
          ...
          if (NON_BAROTROPIC_EOS) {
            ...
            if (MAGNETIC_FIELDS_ENABLED) {
              ...
              cons_(NHYDRO0+IB1,k,j,i) = b1;
              cons_(NHYDRO0+IB2,k,j,i) = b2;
              cons_(NHYDRO0+IB3,k,j,i) = b3;
            }
            cons_(IEN,k,j,i) = e0;
          }
        }
    ...
    //--------------------------------------
    // Currently xorder=4 is not supported (not tested yet)
    //--------------------------------------
    ...
    //   // Compute and store Laplacian of cell-centered conserved variables,
    //   // Hydro and Bcc
    ...
    for (int k=kl; k<=ku; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=il; i<=iu; ++i) {
          ...
          if (MAGNETIC_FIELDS_ENABLED) {
            Real b1 = cons_(NHYDRO0+IB1,k,j,i);
            Real b2 = cons_(NHYDRO0+IB2,k,j,i);
            Real b3 = cons_(NHYDRO0+IB3,k,j,i);
            ...
            l1_err[NHYDRO0 + IB1] += db1*vol;
            l1_err[NHYDRO0 + IB2] += db2*vol;
            l1_err[NHYDRO0 + IB3] += db3*vol;
            max_err[NHYDRO0 + IB1] = std::max(db1, max_err[NHYDRO0+IB1]);
            max_err[NHYDRO0 + IB2] = std::max(db2, max_err[NHYDRO0+IB2]);
            max_err[NHYDRO0 + IB3] = std::max(db3, max_err[NHYDRO0+IB3]);
          }
        }
      }
    }
  ...
  if (Globals::my_rank == 0) {
    MPI_Reduce(MPI_IN_PLACE, &l1_err, (NHYDRO0+NFIELD), MPI_ATHENA_REAL, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(MPI_IN_PLACE, &max_err, (NHYDRO0+NFIELD), MPI_ATHENA_REAL, MPI_MAX, 0,
               MPI_COMM_WORLD);
  } else {
    MPI_Reduce(&l1_err, &l1_err, (NHYDRO0+NFIELD), MPI_ATHENA_REAL, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&max_err, &max_err, (NHYDRO0+NFIELD), MPI_ATHENA_REAL, MPI_MAX, 0,
               MPI_COMM_WORLD);
  }
#endif
  ...
  if (Globals::my_rank == 0) {
    ...
    for (int i=0; i<(NHYDRO0+NFIELD); ++i) l1_err[i] = l1_err[i]/vol;
    // compute rms error
    for (int i=0; i<(NHYDRO0+NFIELD); ++i) {
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      std::fprintf(pfile, "  %e", l1_err[NHYDRO0+IB1]);
      std::fprintf(pfile, "  %e", l1_err[NHYDRO0+IB2]);
      std::fprintf(pfile, "  %e", l1_err[NHYDRO0+IB3]);
    }
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      std::fprintf(pfile, "  %e", max_err[NHYDRO0+IB1]);
      std::fprintf(pfile, "  %e", max_err[NHYDRO0+IB2]);
      std::fprintf(pfile, "  %e", max_err[NHYDRO0+IB3]);
    }
    std::fprintf(pfile, "\n");
    std::fclose(pfile);
  }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    // wave amplitudes
    dby = amp*rem[NWAVE0-2][wave_flag];
    dbz = amp*rem[NWAVE0-1][wave_flag];
...
void MyDualEnSwitchingFunc(Real &ei_nonc,
          const Real &ei_cons, const Real &ekin, const Real &emag) {
  // Nothing is done
  // to always use eint from non-conservative equation

  // The following line is for test.
  // The results must be the same as those for conservative form
  // ei_nonc = ei_cons;
  return;
}
