...
FieldDiffusion::FieldDiffusion(MeshBlock *pmb, ParameterInput *pin) :
  ...
  if (field_diffusion_defined && DUAL_ENERGY_ENABLED) {
    if (!STS_ENABLED) {
      std::stringstream msg;
      msg << "### FATAL ERROR in FieldDiffusion" << std::endl
          << "Magnetic diffusion terms should be solved using STS" << std::endl
          << "when dual energy formalism is used" << std::endl;
      ATHENA_ERROR(msg);
    }
    if (DUAL_ENERGY_FLAG == 0) {
      std::stringstream msg;
      msg << "### FATAL ERROR in FieldDiffusion" << std::endl
          << "DUAL_ENERGY_FLAG must be 1 "
          << "when field diffusion terms are solved" << std::endl;
      ATHENA_ERROR(msg);
    }
  }
}
