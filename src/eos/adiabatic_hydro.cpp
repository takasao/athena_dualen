...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        if (DUAL_ENERGY_ENABLED == 0) {
          ...
          // use internal energy to calculate pressure.
          // do not alter total energy here.
          // floor is already applied to internal energy
          // in ApplyDualEnergyFormalism.
          w_p  = gm1 * cons(IEI,k,j,i);
          Real& w_ei  = prim(IEI,k,j,i);
          w_ei = w_p/gm1;
        }
      }
    }
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        if (DUAL_ENERGY_ENABLED) cons(IEI,k,j,i) = prim(IEI,k,j,i);
      }
    }
  }
...
void EquationOfState::ApplyPrimitiveFloors(AthenaArray<Real> &prim, int k, int j, int i) {
  ...
  if (DUAL_ENERGY_ENABLED) {
    Real& w_ei = prim(IEI,i);
    Real gm1 = GetGamma() - 1.0;
    w_ei= (w_p > pressure_floor_) ? w_ei : (pressure_floor_/gm1);
  }
  w_p = (w_p > pressure_floor_) ?  w_p : pressure_floor_;
...
void EquationOfState::ApplyPrimitiveConservedFloors(
  ...
  if (DUAL_ENERGY_ENABLED) {
    Real& w_ei  = prim(IEI,k,j,i);
    Real& u_ei  = cons(IEI,k,j,i);
    u_ei = (w_p > pressure_floor_) ?
        u_ei : (pressure_floor_/gm1);
    w_ei = (w_p > pressure_floor_) ?
        w_ei : (pressure_floor_/gm1);
  }
