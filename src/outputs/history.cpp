...
// NEW_OUTPUT_TYPES:
  ...
  #define NHYDRO0 (NHYDRO-1) // remove ein
#else // DUAL_ENERGY_ENABLED is OFF
  #define NHYDRO0 (NHYDRO)
#endif // DUAL_ENERGY_ENABLED

// "3" for 1-KE, 2-KE, 3-KE additional columns (come before tot-E)
#define NHISTORY_VARS ((NHYDRO0) + (SELF_GRAVITY_ENABLED > 0) + (NFIELD) + 3 + (NSCALARS))
        ...
        for (int j=pmb->js; j<=pmb->je; ++j) {
          ...
          for (int i=pmb->is; i<=pmb->ie; ++i) {
            ...
            if (SELF_GRAVITY_ENABLED) {
              ...
              hst_data[NHYDRO0 + 3] += vol(i)*0.5*u_d*phi;
            }
            ...
            if (MAGNETIC_FIELDS_ENABLED) {
              ...
              constexpr int prev_out = NHYDRO0 + 3 + (SELF_GRAVITY_ENABLED > 0);
            ...
            for (int n=0; n<NSCALARS; n++) {
              ...
              constexpr int prev_out = NHYDRO0 + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD;
