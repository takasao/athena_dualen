...
class Hydro {
 ...
 public:
  ...
  // START:: dual energy formalism
  // velocity vectors derived from the Riemann solver
  AthenaArray<Real> vf[3];
  AthenaArray<Real> divV_;
  void InitializeDualEnergyFormalism();
  void AnalyticIntegrationDualEnergyFormalism(AthenaArray<Real> &u,
                                    int il, int iu, int jl, int ju, int kl, int ku,
                                    const Real wght);
  // IEI in prim will not be updated here because prim stores old timestep data.
  void ApplyDualEnergyFormalism(AthenaArray<Real> &cons);
  void SynchronizeEintWithEtot(AthenaArray<Real> &prim, AthenaArray<Real> &cons,
                            AthenaArray<Real> &bcc,
                            int il, int iu, int jl, int ju, int kl, int ku);
  AthenaArray<Real> bcc_wrk_;
  DualEnergyFunc UserDualEnergy;
  // END:: dual energy formalism
