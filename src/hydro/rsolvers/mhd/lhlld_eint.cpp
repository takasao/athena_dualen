...
//!        This is compatible with dual energy formalism.
...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  Real vf;                        // Velocity at the cell face for dual energy formalism
...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli,wri,spd,flxi,vf)
  for (int i=il; i<=iu; ++i) {
    ...
    ulst.ei= ul.ei* sdl * sdml_inv;
    urst.ei= ur.ei* sdr * sdmr_inv;
    ...
    uldst.ei= spd[1] * (uldst.ei- ulst.ei); // same as density
    ...
    ulst.ei= spd[0] * (ulst.ei- ul.ei); // same as density
    ...
    urdst.ei= spd[3] * (urdst.ei- urst.ei); // same as density
    ...
    urst.ei= spd[4] * (urst.ei - ur.ei); // same as density
    ...
    if (spd[0] >= 0.0) {
      ...
      flxi[IEI] = fl.ei + ulst.ei + uldst.ei;
      vf = spd[2];
    } else if (spd[3] > 0.0) {
      // return Fr**
      ...
      flxi[IEI] = fr.ei + urst.ei + urdst.ei;
      vf = spd[2];
    } else {
      // return Fr*
