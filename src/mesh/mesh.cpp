...
//----------------------------------------------------------------------------------------
//! \fn void Mesh::EnrollUserDualEnergySwitchingFunction(DualEnFunc my_func)
//! \brief Enroll a user-defined source function

void Mesh::EnrollUserDualEnergySwitchingFunction(DualEnergyFunc my_func) {
  UserDualEnergy_ = my_func;
  return;
}
...
#pragma omp parallel for num_threads(nthreads)
    ...
    if (res_flag == 0 && DUAL_ENERGY_ENABLED) {
#pragma omp parallel for num_threads(nthreads)
      for (int i=0; i<nblocal; ++i) {
        MeshBlock *pmb = my_blocks(i);
        pmb->phydro->InitializeDualEnergyFormalism();
      }
    }
